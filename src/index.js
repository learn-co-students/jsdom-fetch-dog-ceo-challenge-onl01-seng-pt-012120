// runs function once website is loaded and fully parsed
document.addEventListener('DOMContentLoaded', () => {
    fetchDogs()
    fetchBreeds()
})

// renders json function
// saves dog image container in variable
// iterates through images and place then in dog-image-container section
function renderDogs(json) {
    const dogImageContainer = document.getElementById('dog-image-container')
    json.message.forEach(dog => {
        dogImageContainer.innerHTML += `<img src = '${dog}'>`
    })
}

// fetches data from url
function fetchDogs() {
    return fetch('https://dog.ceo/api/breeds/image/random/4')
    .then(resp => resp.json())
    .then(json => renderDogs(json)) // <= renders function and passes api data
}

// fetches data from url
function fetchBreeds() {
    return fetch('https://dog.ceo/api/breeds/list/all')
    .then(resp => resp.json())
    .then(json => renderBreeds(json))  // <= renders function and passes api data
}

// renders json function
// saves dog objs from api to variable
// iterates through hash and place then in dog-image-container section
function renderBreeds(json) {
    const dogBreedContainer = document.querySelector('#dog-breeds')
    const dogObj = json.message
    Object.keys(dogObj).forEach(dog => {
        dogBreedContainer.innerHTML += `<li id='${dog.charAt(0)}'>${dog}</li>`
    })
    textColor()
    
    let dropDown = document.querySelector('#breed-dropdown')

    dropDown.addEventListener('change', (e) => {
        let breedChildren = document.querySelector('#dog-breeds').children
        let breedsArray = [...breedChildren]
        breedsArray.forEach(breed => {
            if (breed.id === e.target.value) {
                breed.style.display = null      
            } else {
                breed.style.display = 'none'
            }
            }) 
        })
    }
    
    function textColor() { 
        let dogBreedContainer = document.querySelector('#dog-breeds')
        dogBreedContainer.addEventListener("click", (e) => {
            e.target.style.color = '#69b9ff'       
        })
    }
